...
//Konfig Struktur McBSP0			-----------HIER csl_sru401.pdf
static MCBSP_Config mcbspforAIC23Cfg = {
        ...
        MCBSP_FMKS(SPCR, FRST, YES)            	|	// Frame-sync generator reset bit 					--- ?? aktive clkg clock
        ...
        MCBSP_FMKS(SPCR, XRST, YES)             |	// Transmitter reset bit resets or enables the transmitter. ----?? ruhe zustand
...
{
	/* Handle für Steuerkanal */	//-----------HIER script use of the csl
	MCBSP_Handle hMcbsp0;
	unsigned short i;

	hMcbsp0 = MCBSP_open(MCBSP_DEV0, MCBSP_OPEN_RESET); 							//---siehe script
    MCBSP_config(hMcbsp0,&mcbspforAIC23Cfg); // sieheoben beschriebene config
    MCBSP_start(hMcbsp0, MCBSP_RCV_START | MCBSP_XMIT_START | MCBSP_SRGR_START | MCBSP_SRGR_FRAMESYNC, 220); 			//siehe skript
...
void set_aic23_register( MCBSP_Handle hMcbsp, unsigned short regnum, unsigned short regval)
{
    ...
    /* warten */
    while (!MCBSP_xrdy(hMcbsp)); // in datenblatt mit if, hier mit while
								 // warten solange bis fertig, deswegen ! (soll ja warten)
    
    /* schreiben */
    MCBSP_write(hMcbsp, (regnum << 9)| regval );  // regnumm =9
												  // bitweise mit regval neuen wert zuweisen
